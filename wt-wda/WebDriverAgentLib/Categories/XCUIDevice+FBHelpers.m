...
- (NSData *)fb_fastscreenshotWithError:(CGFloat)width  height:(CGFloat)height reror:(NSError*__autoreleasing*)error
{
  ...
  double start0 = [[NSDate date] timeIntervalSince1970]*1000;
  ...
  NSLog(@"screenshotDataForQuality use time =%f millisecond ", (end-start));
  ...
  double start1 = [[NSDate date] timeIntervalSince1970]*1000;
//  UIImage *image = [UIImage imageWithData:result];
// NSData * data=UIImagePNGRepresentation(image);
  double end0 = [[NSDate date] timeIntervalSince1970]*1000;
  NSLog(@"screenshot post use time =%f millisecond ", (end0-start1));
  NSLog(@"screenshot whole use time =%f millisecond ", (end0-start0));
  return result;
}
...
- (NSData *)fb_screenshotWithError:(NSError*__autoreleasing*)error
{
  ...
  double end2 = [[NSDate date] timeIntervalSince1970]*1000;
  // https://developer.apple.com/documentation/xctest/xctimagequality?language=objc
  // Select lower quality, since XCTest crashes randomly if the maximum quality (zero value) is selected
  // and the resulting screenshot does not fit the memory buffer preallocated for it by the operating system
  NSUInteger quality = 2;
  ...
  NSData * data=UIImagePNGRepresentation(image);
  double end0 = [[NSDate date] timeIntervalSince1970]*1000;
  NSLog(@"screenshot pre use time =%f millisecond ", (end2-start0));
  NSLog(@"screenshot post use time =%f millisecond ", (end0-start1));
  NSLog(@"screenshot whole use time =%f millisecond ", (end0-start0));
  return data;
}
