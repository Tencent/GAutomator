...
class Client(object):
    ...
    def screenshot(self, png_filename=None):
        ...
        print("screenshot cost: "+str((time.time()-starttime)*1000)+"ms")
        raw_value = base64.b64decode(value)
        png_header = b"\x89PNG\r\n\x1a\n"
        if not raw_value.startswith(png_header):
            raise WDAError(-1, "wda screenshot png format error")
...
class Session(object):
    ...
    def check_alert(self):
        for _ in range(0,3):
            if self.alert.exists and self.http.alert_callback:
